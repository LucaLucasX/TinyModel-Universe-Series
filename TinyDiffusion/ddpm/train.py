...
def run_step(model, dataloader, noise_scheduler, criterion, epoch, num_epochs, device):
    """测试步骤,计算测试集上的损失"""
    model.eval()
    with torch.no_grad():
        loss_sum = 0
...
def train(model, train_loader, test_loader, noise_scheduler, criterion, optimizer, device, num_epochs=100, img_size=32):
    ...
    for epoch in range(num_epochs):
        ...
        test_loss = run_step(model, test_loader, noise_scheduler, criterion, epoch, num_epochs, device)
        if epoch % 10 == 0:
            # 采样10张图像
...
if __name__ == "__main__":
    ...
    train_loader, test_loader = load_transformed_dataset(img_size=args.img_size, batch_size=args.batch_size)
